...
.. code-block:: bash
    ...
    --output_file ./motions/0007_Walking001_stageii.npz --output_fps 50 --headless

    python ths_t1/offline_csv_to_npz.py --input_file data/0005_Jogging001_stageii.csv --input_fps 30 --frame_range 122 722 \
    --output_file ./motions/0005_Jogging001_stageii.npz --output_fps 50 --headless
"""
...
class MotionLoader:
    ...
    def _compute_velocities(self):
            """Computes the velocities of the motion - 修改为保持零速度"""
            ...
            # 创建零角速度张量
            # 假设旋转用四元数表示 (N, 4)，则角速度应为 (N, 3)
            if len(self.motion_base_rots.shape) == 2 and self.motion_base_rots.shape[1] == 4:
                self.motion_base_ang_vels = torch.zeros(
                    (self.motion_base_rots.shape[0], 3), 
                    device=self.motion_base_rots.device, 
                    dtype=self.motion_base_rots.dtype
                )
            else:
                # 计算一次以获取正确的形状，然后置零
                ang_vels = self._so3_derivative(self.motion_base_rots, self.output_dt)
                self.motion_base_ang_vels = torch.zeros_like(ang_vels)
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        joint_vel[:, robot_joint_indexes] = motion_dof_vel  #   joint_vel
        ...
        #     print(f"Body '{body_name}': position={pos}, orientation={quat}")

        # # 获取躯干索引,打印特定body 
        # torso_index = robot.body_names.index("torso_link")
        # torso_pos = robot.data.body_pos_w[0, torso_index].cpu().numpy()
        # torso_quat = robot.data.body_quat_w[0, torso_index].cpu().numpy()
        # print(f"Torso position: {torso_pos}, orientation: {torso_quat}")
        ########################
